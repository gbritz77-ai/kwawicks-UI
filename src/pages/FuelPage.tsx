...
function VehicleReportCard({ v }: { v: VehicleFuelReportDto }) {
  ...
  const borderColor = v.isOutOfRange ? "#fca5a5" : "#e2e8f0";
  const bg = v.isOutOfRange ? "#fff5f5" : "#fff";

  return (
    <div style={{ ...s.card, borderColor, background: bg }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {v.isOutOfRange && (
              <span style={{ ...s.badge, background: "#fee2e2", color: "#991b1b" }}>⚠ High Consumption</span>
            )}
          </div>
          {v.isOutOfRange && v.topIssuedBy && (
            <div style={{ fontSize: 12, color: "#dc2626", marginTop: 2 }}>
              Driver/issuer: <strong>{v.topIssuedBy}</strong>
            </div>
          )}
        </div>
      ...
      <div style={s.statsRow}>
        ...
        {v.avgConsumptionPer100 != null && (
          <Stat
            label={`L/100${v.odoType}`}
            value={`${v.avgConsumptionPer100.toFixed(1)}`}
            highlight={v.isOutOfRange ? "red" : undefined}
          />
        )}
        {v.expectedConsumption != null && (
          <Stat label="Expected" value={`${v.expectedConsumption} L/100${v.odoType}`} dim />
        )}
      </div>
...
function Stat({ label, value, highlight, dim }: { label: string; value: string; highlight?: "red"; dim?: boolean }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
      <div style={{ fontSize: 10, color: "#94a3b8", textTransform: "uppercase" as const, letterSpacing: "0.05em" }}>{label}</div>
      <div style={{ fontSize: 14, fontWeight: 700, color: highlight === "red" ? "#dc2626" : dim ? "#94a3b8" : "#1e293b" }}>{value}</div>
    </div>
  );
}
...
export default function FuelPage() {
  ...
  async function loadReport() {
    ...
    try {
      setReport(await fuelIssuesApi.getReport(reportFrom || undefined, reportTo || undefined));
  ...
  return (
    <div style={s.page}>
      <div style={s.header}>
        ...
        {tab === "report" && (
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" as const }}>
            <input style={{ ...s.input, width: 140 }} type="date" value={reportFrom} onChange={e => setReportFrom(e.target.value)} />
            <input style={{ ...s.input, width: 140 }} type="date" value={reportTo} onChange={e => setReportTo(e.target.value)} />
      ...
      {tab === "issues" && (
        ...
        issues.length === 0 ? <div style={s.emptyText}>No fuel issues recorded yet.</div> : (
          <div style={s.tableWrap}>
            <table style={s.table}>
              <thead>
                <tr>
                  {["Date", "Vehicle", "Tank", "Site", "Litres", "Odometer", "Cost/L", "Total", "Issued By", "Slip"].map(h => (
      ...
      {tab === "report" && (
        ...
        !report ? null :
        report.length === 0 ? <div style={s.emptyText}>No fuel data for this period.</div> : (
          <>
            {outOfRange.length > 0 && (
